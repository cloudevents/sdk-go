...
func TestCodecEncode(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"simple v03 structured": {
			codec: kafka.Codec{Encoding: kafka.StructuredV03},
			event: cloudevents.Event{
		...
		"simple v03 binary": {
			codec: kafka.Codec{Encoding: kafka.BinaryV03},
			event: cloudevents.Event{
				Context: &cloudevents.EventContextV03{
					Type:    "com.example.test",
					Source:  *source,
	...
	for n, tc := range testCases {
		t.Run(n, func(t *testing.T) {
			...
			if diff := cmp.Diff(tc.want, got); diff != "" {
				if msg, ok := got.(*kafka.Message); ok {
...
func TestCodecAsMiddleware(t *testing.T) {
	...
	for _, encoding := range []kafka.Encoding{kafka.StructuredV03} {
		...
		testCases := map[string]struct {
			...
			"simple data": {
				codec: kafka.Codec{Encoding: encoding},
				event: cloudevents.Event{
					Context: cloudevents.EventContextV03{
						Type:   "com.example.test",
						Source: *source,
