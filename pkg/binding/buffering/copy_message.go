...
func BufferMessage(m binding.Message, transformers ...binding.TransformerFactory) (binding.Message, error) {
	result, err := CopyMessage(m, transformers...)
	if err != nil {
		return nil, err
	}
...
func CopyMessage(m binding.Message, transformers ...binding.TransformerFactory) (binding.Message, error) {
	...
	if originalMessageEncoding == binding.EncodingEvent {
		e, _, err := binding.ToEvent(m, transformers...)
		if err != nil {
			return nil, err
		}
