...
func TestRetriesNil_Is(t *testing.T) {
	var err error
	if protocol.ResultIs(err, NewRetriesResult(nil, 0, time.Nanosecond, nil)) {
...
func TestRetriesNew_Is(t *testing.T) {
	err := NewRetriesResult(NewResult(200, "this is an example message, %s", "yep"), 0, time.Nanosecond, nil)
...
func TestRetriesNewOtherType_Is(t *testing.T) {
	err := NewRetriesResult(NewResult(404, "this is an example error, %s", "yep"), 0, time.Nanosecond, nil)
	if protocol.ResultIs(err, NewResult(200, "OK")) {
...
func TestRetriesNewWrappedErrors_Is(t *testing.T) {
	err := NewRetriesResult(NewResult(500, "this is a wrapped error, %w", io.ErrUnexpectedEOF), 0, time.Nanosecond, nil)
...
func TestRetriesNewOtherStatus_Is(t *testing.T) {
	err := NewRetriesResult(NewResult(403, "this is an example error, %s", "yep"), 0, time.Nanosecond, nil)
	if protocol.ResultIs(err, NewRetriesResult(NewResult(200, "OK"), 0, time.Nanosecond, nil)) {
		t.Error("Did not expect event to be StatusCode=200")
	}
}

func TestRetriesNew_As(t *testing.T) {
	err := NewRetriesResult(NewResult(404, "this is an example error, %s", "yep"), 5, time.Nanosecond, nil)
...
func TestRetriesNew_Error(t *testing.T) {
	err := NewRetriesResult(NewResult(500, "this is an example error, %s", "yep"), 0, time.Nanosecond, nil)
...
func TestRetriesNew_ErrorWithRetries(t *testing.T) {
	err := NewRetriesResult(NewResult(500, "this is an example error, %s", "yep"), 10, time.Nanosecond, nil)
