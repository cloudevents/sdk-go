...
// requestHandler implements fasthttp.Handler.
// Blocks until ResponseFn is invoked.
func (p *Protocol) requestHandler(rtx *fasthttp.RequestCtx) {
	// Filter the GET style methods:
	switch {
	case rtx.IsOptions():
		if p.OptionsHandlerFn == nil {
			rtx.Response.SetStatusCode(fasthttp.StatusMethodNotAllowed)
			return
		}
		// TODO
	...
	if m == nil {
		...
		rtx.Response.SetStatusCode(fasthttp.StatusBadRequest)
	...
	var fn protocol.ResponseFn = func(ctx context.Context, respMsg binding.Message, res protocol.Result, transformers ...binding.Transformer) error {
		...
		if finishErr != nil {
			rtx.Response.SetStatusCode(fasthttp.StatusInternalServerError)
			rtx.Response.Header.SetContentType("text/plain; charset=utf-8")
			_, _ = fmt.Fprintf(rtx, "Cannot forward CloudEvent: %s", finishErr)
			return finishErr
		}
		...
		if res != nil {
			...
			case !protocol.IsACK(res):
				...
				if errors.As(res, &validationError) {
					rtx.Response.SetStatusCode(fasthttp.StatusBadRequest)
					rtx.Response.Header.SetContentType("text/plain; charset=utf-8")
					_, _ = fmt.Fprintf(rtx, validationError.Error())
					return validationError
				} else if errors.Is(res, binding.ErrUnknownEncoding) {
					status = fasthttp.StatusUnsupportedMediaType
				} else {
		...
		rtx.Response.SetStatusCode(status)
		return nil
	}
