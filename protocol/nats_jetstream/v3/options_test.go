...
func TestWithURL(t *testing.T) {
	...
	natsOptions := []nats.Option{userJWTAndSeed, secure}
	type args struct {
		protocol *Protocol
		url      string
		options  []nats.Option
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			gotErr := tt.args.protocol.applyOptions(WithURL(tt.args.url, tt.args.options...))
...
func TestWithConsumerConfig(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "too many consumer options",
			args: args{
				protocol: &Protocol{orderedConsumerConfig: &jetstream.OrderedConsumerConfig{FilterSubjects: filterSubjects}},
				config:   &jetstream.ConsumerConfig{FilterSubjects: filterSubjects},
			},
			wants: wants{
...
func TestWithOrderedConsumerConfig(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			name: "too many consumer options",
			args: args{
				protocol: &Protocol{consumerConfig: &jetstream.ConsumerConfig{FilterSubjects: filterSubjects}},
				config:   &jetstream.OrderedConsumerConfig{FilterSubjects: filterSubjects},
			},
			wants: wants{
				err:      ErrMoreThanOneConsumerConfig,
...
func TestWithPullConsumeOptions(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "no pull consumer option given",
			args: args{
				protocol: &Protocol{},
				config:   nil,
			},
