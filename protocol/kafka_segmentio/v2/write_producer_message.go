...
func WriteProducerMessage(ctx context.Context, m binding.Message, producerMessage *kafka.Message, transformers ...binding.Transformer) error {
...
func (b *kafkaProducerMessageWriter) SetAttribute(attribute spec.Attribute, value interface{}) error {
	if attribute.Kind() == spec.DataContentType {
		...
		b.Headers = append(b.Headers, protocol.Header{Key: contentTypeHeader, Value: []byte(s)})
	} else {
		if value == nil {
		...
		b.Headers = append(b.Headers, protocol.Header{Key: prefix + attribute.Name(), Value: []byte(s)})
	}
	return nil
}
...
func (b *kafkaProducerMessageWriter) SetExtension(name string, value interface{}) error {
	...
	b.Headers = append(b.Headers, protocol.Header{Key: prefix + name, Value: []byte(s)})
	return nil
}
